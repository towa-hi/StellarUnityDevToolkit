...
namespace Stellar.Utilities
{
    ...
    public class StrKey
    {
        ...
        {
            if (muxedAccount is MuxedAccount.KeyTypeEd25519 a)
            {
                return EncodeCheck(VersionByte.ACCOUNT_ID, a.ed25519.InnerValue);
            
            }
            if (muxedAccount is MuxedAccount.KeyTypeMuxedEd25519 ma)
            {
                var idBytes = BitConverter.GetBytes(ma.med25519.id.InnerValue).ToBigEndian();
                var bytes = ma.med25519.ed25519.InnerValue.Concat(idBytes).ToArray();
                return EncodeCheck(VersionByte.MUXED_ACCOUNT, bytes);
            }
            throw new ArgumentException("Unsupported Muxed Account type.");
